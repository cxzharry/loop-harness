...
`.loop-harness/product-loop-run-log.md` contains a recent failed or regressed iteration.

## Expected Behavior

- Classify the failure as one of the supported error classes.
- Append raw evidence to `.loop-harness/product-loop-run-log.md`.
...
- Promote durable facts into `.loop-harness/PRODUCT_LOOP_STATE.md`.
- Create or update an active `Regression Case` in `.loop-harness/PRODUCT_LOOP_BENCHMARK.md`.
